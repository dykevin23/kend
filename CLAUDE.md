...
- **타 패키지 진행상황**은 그 패키지 changelog(`changelog-native.md`, `changelog-seller.md` — 이 repo에도 복사본 존재)로 참조한다. changelog 3종의 프로젝트 간 sync는 **`bash scripts/sync-changelogs.sh`** 로 처리(각 패키지 원본을 3개 repo에 복사). 원본(canonical): changelog-kend←kend, native←kend-native, seller←kend-seller.
