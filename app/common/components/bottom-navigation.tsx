...
const NaviMenuItem = ({ url, name, icon, isMatch }: NaviMenuItemProps) => {
  return (
    <Link to={url}>
      <div className="flex flex-col w-[72px] items-center gap-2">
        {icon}
        <span className={cn(["text-xs leading-3", isMatch && "text-primary"])}>
          {name}
        </span>
      </div>
    </Link>
