...
- **리뷰 관리** (구 P2-10 잔여, kend-seller)
- ✅ **리뷰 작성 기능 자체 (kend) — 구현·실사용 테스트 중 (2026-09-02)** — `reviews` 테이블 신설(user_id+product_id 유니크, 구매확정자만 상품당 1개, 별점+텍스트 MVP). 주문상세·주문목록(배송·주문 관리 옆, 반씩 분할)에 "리뷰 작성" 버튼, `/myPage/reviews/new` 작성 폼, `/myPage/reviews` 내가 쓴 리뷰 목록(마이페이지 퀵메뉴 재노출) 신규. 상품페이지 가짜 평점(4.5/4,321)·가짜 리뷰·리뷰(4,321) 탭카운트를 전부 실데이터로 교체, 색감/사이즈/두께감 만족도는 데이터 없어 제거(백로그). 실사용 테스트 중 발견 수정: ①작성 후 리다이렉트를 `navigate(-1)` 방식에서 `returnTo` 쿼리파라미터를 액션에 직접 넘겨 서버 `redirect()`로 처리하도록 변경 — `navigate(-1)`는 제출 성공 후 자동 재검증되는 loader의 "이미 작성함" 가드가 먼저 리다이렉트를 가로채 항상 주문상세로 튕기는 버그가 있었음 ②이미 리뷰를 쓴 상품은 버튼이 사라지는 대신 "내가 쓴 리뷰 보기"(`/myPage/reviews`)로 전환 ③리뷰카드 날짜 줄바꿈 버그, "등록 | 날짜" → "날짜 등록" 문구 변경 ④평점/리뷰 섹션 빈 상태 문구 중복 표시 ⑤"전체" 탭(`OrderGroupCard`)의 배송완료+구매확정 건도 "배송 조회" 대신 리뷰 작성/보기 버튼으로 전환(주문목록·주문상세와 동일 로직) ⑥"리뷰 보기"가 전체 리뷰 목록으로 가던 것을 `/myPage/reviews?productCode=`로 필터링해 해당 상품 리뷰만 보이게 수정 ⑦같은 상품을 여러 번 구매했을 때, 이번 주문 건이 구매확정 전인데도 "다른 주문 건에서 이미 리뷰함"만으로 리뷰보기가 뜨던 버그 — 리뷰 버튼(작성/보기) 대상을 "이번 delivery_item 자체가 normal+구매확정"인 것으로 한정하도록 3곳(주문상세/주문목록/전체탭) 전부 수정
- ⚠️ **네비게이션 버그 수정(2026-09-03/04)**: ①작성 성공 후 리다이렉트를 `fetcher.Form`으로 처리하고 있었는데, fetcher의 리다이렉트는 히스토리에 새로 쌓여서(push) 뒤로가기 시 이미 작성된 리뷰작성 화면으로 갔다가 loader 가드에 의해 즉시 다시 튕겨나오는 루프가 생겼음 — 일반 네비게이션 `Form`(react-router)으로 교체, 액션 리다이렉트가 replace로 처리되게 수정 ②"리뷰 작성" 링크로 이동하는 것 자체는 push라서, 제출 후 replace 리다이렉트로 돌아와도 주문화면이 한 겹 더 끼어 뒤로가기가 한 번에 안 먹히던 문제 — "리뷰 작성" 링크(제출→리다이렉트로 이어지는 경로만)에 `replace` 추가해 깊이를 원래대로 유지. "리뷰 보기"(단순 조회, 리다이렉트 없음)는 기존 push 유지
- ⚠️ **정책 변경(2026-09-03)**: "상품당 1개(평생)" → **"구매확정 건(delivery_item)당 1개"** — 재구매 시점마다 경험이 다를 수 있고(품질/배송/옵션 변화), 리뷰 총량이 플랫폼에 유리하다는 일반적인 이커머스 관행(쿠팡/무신사 등)에 맞춤. `reviews.delivery_item_id` 컬럼 신설 + unique 제약을 `(user_id, product_id)`에서 `delivery_item_id` 단일로 변경(테스트 데이터 2건 정리 후 마이그레이션). 관련 쿼리/버튼 로직 전부 `productId` 기준 → `deliveryItemId` 기준으로 재작성 완료
- ✅ **리뷰 이미지 업로드 (2026-09-04)** — kend-seller 리뷰관리 화면(이미지/답변/통계) 참고 요청에 따라 kend 쪽 이미지 첨부 기능 추가. `review_images` 테이블 신설(product_images와 동일한 분리 테이블 패턴), `reviews` 스토리지 버킷 신규 생성(공개 읽기, 5MB, jpeg/png/webp/gif) + RLS 정책(경로 첫 세그먼트=`auth.uid()` 소유권 검증, `profiles` 버킷과 동일 패턴). 경로: `reviews/{userId}/{deliveryItemId}/{randomId}.{ext}`. 작성 폼에 최대 5장 첨부(클라이언트에서 직접 업로드 후 URL을 hidden input으로 제출), 상품페이지·내 리뷰 목록에 썸네일 노출.
- ⚠️ **다건 주문 액션 버그 4종 수정 (2026-09-04)**: ①`order-detail-page.tsx`에서 fetcher를 화면 전체가 공유해 상품 하나 구매확정 시 다른 상품 버튼까지 "처리 중"으로 번지던 버그 — 버튼별 독립 fetcher로 분리(`ConfirmPurchaseButton` 컴포넌트화) ②주문 안 상품이 전부 정상 상태면 개별 확정 대신 "전체 구매확정" 버튼 하나로 미확정 건을 한 번에 처리(`confirmPurchaseBulk` 액션 신설), 상태가 하나라도 갈리면(반품 등) 그 순간부터 개별 버튼으로 복귀 ③주문목록/전체탭에서 리뷰 작성/보기 대상이 여러 개인데 버튼 하나가 그중 1개만 연결해 나머지 상품은 리뷰를 쓸 방법이 없던 버그 — "대상 1개면 바로 연결, 여러 개면 주문상세로 보내서 상품별로 처리" 원칙으로 통일 ④다건 주문에서 "리뷰 작성 (N)" 버튼으로 주문상세에 진입하는 이동 자체가 push라서, 그 뒤 개별 작성→제출→replace 복귀를 거쳐도 주문상세 화면이 한 겹 더 끼어 뒤로가기가 한 번에 안 되던 문제 — 이 진입 링크에도 `replace` 추가(단건 "리뷰 작성" 때와 동일한 원인·동일한 해법) *(→ 아래 2026-09-04 실사용 테스트 피드백 반영분에서 이 ④ 조치는 되돌림)*
- ✅ **리뷰 기능 전체 테스트 시나리오 7종 통과 (2026-09-04)** — 작성 기본플로우/정책검증/이미지첨부/조회/판매자답변/다건주문/버튼노출 전부 사용자 실사용 테스트 통과. 이후 발견된 5건 피드백은 아래 항목에서 처리
- ✅ **테스트 피드백 5건 반영 (2026-09-04)**: ①구매확정(개별+전체) 버튼을 주문상세뿐 아니라 주문목록(배송중/배송완료/취소환불 탭 `order-status-card.tsx`, 전체 탭 `order-group-card.tsx`)에도 동일하게 노출 — `getConfirmPurchaseGroup` 공유 유틸로 3개 화면 로직 통일 ②구매확정(개별+전체) 클릭 시 확인창 추가 ③리뷰 작성 폼에서 제출 없이 뒤로가기 시 원래 있던 화면이 아니라 마이페이지로 튀던 문제 — 원인은 "리뷰 작성" 진입 링크에 걸려 있던 `replace`가 진입 시점에 원본 화면의 히스토리 엔트리 자체를 지워버린 것. 진입 링크를 다시 일반 push로 되돌려 취소 시 정확히 이전 화면으로 돌아가도록 임시 수정(뒤로가기가 아예 안 먹히는 문제보다 우선 해소, 이후 ⑥에서 근본 해결) ④작성 폼 상단에 상품명/이미지만 있던 것에 옵션·판매가 추가 표시 ⑤같은 이미지를 실수로 중복 선택하는 것을 파일명+용량 기준으로 감지해 안내 문구 표시(내용 해시 비교까지는 과한 처리로 판단해 제외)
- ✅ **구매확정 버튼 위치 조정 (2026-09-07)**: 주문목록/전체탭에서 상품 카드 바로 아래 있던 구매확정 버튼을 "배송·주문 관리"/"배송 조회" 액션 버튼 행 아래, 너비 100% 버튼으로 이동(개별·전체 확정 모두 동일)
- ✅ **리뷰상세 옵션·가격 표시 (2026-09-07)** — 내가 쓴 리뷰(`/myPage/reviews`)에도 작성폼과 동일하게 구매 시점 옵션·판매가 노출(`reviews.delivery_item_id → delivery_items → order_items` 체인으로 조회)
- ✅ **리뷰 작성 — 페이지 이동 → 전체화면 팝업(Modal)으로 전환, 뒤로가기 문제 근본 해결 (2026-09-07)**: 위 ③의 push/replace 트레이드오프 자체를 없애기 위해, "리뷰 작성"을 `/myPage/reviews/new` 라우트 이동 방식에서 장바구니 결제·배송지 추가와 동일한 패턴(`Modal` 공통 컴포넌트 + `fetcher.Form`, 페이지 이동 없음)으로 전환. 취소=팝업 닫기(현재 화면 그대로라 뒤로가기 개입 자체가 없음), 등록 성공=팝업 닫고 `useRevalidator()`로 현재 페이지 데이터만 갱신. `ReviewWriteButton` 컴포넌트 신설(주문상세/주문목록/전체탭 3곳 공용, 이미 로드된 주문 아이템 정보로 상품명·옵션·가격 표시해 별도 조회 불필요), 작성 액션은 `/myPage/reviews/action`(action-only 리소스 라우트, `order-action.ts`와 동일한 intent 기반 패턴)으로 분리. `review-new-page.tsx`(라우트+로더) 삭제, `getProductForDeliveryItem` 쿼리 제거(더 이상 불필요), `uploadReviewImage`는 `userId`를 인자로 안 받고 내부에서 `auth.getUser()`로 조회하도록 정리(호출부에서 프롭 드릴링 불필요)
**답변/통계/날짜검색 등 나머지는 kend-seller 담당**
- ✅ **판매자 답변 스키마 추가 (2026-09-04)** — `reviews.seller_reply`(text, nullable), `reviews.seller_replied_at`(timestamptz, nullable) 컬럼 신설. `seller_replied_at IS NULL`이 곧 "미답변" 판정 기준. kend 쪽 조회(상품페이지 리뷰 목록, 내 리뷰 목록)에도 판매자 답변 노출 반영 완료. kend-seller는 `db:typegen` 재실행 후 이 두 컬럼에 UPDATE하는 방식으로 답변 기능 구현하면 됨(별도 테이블 불필요)
- **최근 본 상품 (kend, 신규 발견 2026-09-02)** — `recent-products-page.tsx`가 항상 "최근 본 상품이 없습니다" 고정 표시하는 정적 스텁, 열람 이력 저장/조회 로직 없음
- **상품 평점/만족도 가짜 데이터 (kend, 신규 발견 2026-09-02)** — `product-rating-section.tsx`가 평점 4.5·리뷰수 4,321·색감/사이즈/두께감 만족도까지 전부 하드코딩 기본값, 모든 상품이 동일하게 표시됨. 리뷰 작성 기능(위 항목) 선행 필요
- **상품 사이즈표 가짜 데이터 (kend, 신규 발견 2026-09-02)** — `product-size-description.tsx`가 상품과 무관하게 고정 사이즈표(12M/24M/36M) 표시. 상품/SKU별 사이즈 데이터 자체가 스키마에 없음
- ✅ **찜 목록 "스토어" 탭 — 완료, 실사용 테스트 완료 (2026-09-02)** — `store_likes` 테이블 신설(`product_likes`와 동일 패턴, user_id+seller_id 복합키). 스토어 페이지에 찜하기 버튼 신규 추가(기존엔 찜할 방법 자체가 없었음), 찜 목록에 제외(X) 버튼 추가, `LikeStoreCard` 신규. 실사용 중 발견된 버그 수정: ①`makeCachedClientLoader`(iOS 스와이프백 대응 캐시) mutation 후 무효화 로직이 없어 찜 상태가 stale하게 남는 문제 — `invalidate()` 메서드 추가 ②찜 카드 행이 `pb-6`(아래쪽만 패딩)이라 콘텐츠가 위로 쏠려 보이던 문제, 상품찜 목록도 동일 원인이라 같이 수정 ③로고 이미지 배경 없어서 비정사각형 로고가 작아 보이던 문제 ④`Tabs` 공통 컴포넌트에 비활성 탭 하단 구분선이 없던 것 발견해 전체 반영. 찜 버튼 색은 상품페이지와 통일 시도했다가(핑크) 반려, 최종 `secondary`로 확정
- 🔀 **공지사항 화면 — kend-seller 선행 필요 (2026-09-02)** — `notices` 테이블 자체 없음, 항상 빈 상태. 관리자가 데이터를 넣는 CRUD 게시판 성격이라 **kend-seller가 스키마+admin CRUD 화면을 먼저 만들고, kend은 그 데이터를 읽어서 보여주는 화면만 담당** (문의하기 P2.5-4와 같은 소유권 원칙). kend-seller 작업 전까지 kend 쪽은 착수 불가
- ⏸ **알림 설정 저장 안 됨 — 백로그 (2026-09-02)** — 토글이 로컬 state만 바꾸고 새로고침하면 초기화, DB 컬럼 없음. kend에 푸시 발송 인프라 자체가 없어(토큰 등록/발송 로직 전무) kend-native 푸시 기능 도입과 같이 진행해야 함, 단독 작업 아님
- ✅ **마이페이지 죽은 링크 — 완료 (2026-09-02)** — 리뷰/쿠폰/포인트/마일리지 퀵메뉴 제거(연결된 라우트 자체가 없었음). 리뷰는 작성 기능 생기면 재추가
- ⏸ **팔로워 수 미연동 — 백로그, 드롭 후보 (2026-09-02)** — `follows` 테이블은 있고 `getStoresWithProducts`/`getStoreByCode`가 `followerCount: 0`으로 하드코딩돼있으나, **팔로우 개념 자체가 서비스에서 없어질 가능성**이 있어 연동 작업 보류. 팔로우 유지로 결정되면 그때 가볍게 처리(테이블은 이미 있음)
- ⏸ **"현재 위치로 주소 찾기" 버튼 미동작 — 백로그 (2026-09-02)** — 체크아웃 주소입력에서 클릭해도 무반응, 네이티브 기능 연동 TODO만 있고 구현 없음. kend-native 위치 권한/좌표 전달 브릿지 선행 필요해서 kend 단독으로는 해결 불가
- ✅ **경미 항목 — 완료 (2026-09-02)** — 구매모달 발송예정일 고정문구를 실제 SLA 기준 문구(판매자 확인 후 3영업일)로 수정, 장바구니 배송비를 결제모달과 동일한 판매자별 그룹핑 로직(`groupOrderItemsBySeller`)으로 실제 계산되게 수정(요약 총액 + 배송비 표시 줄 둘 다). 쿠폰 UI는 쿠폰 시스템 자체가 없어 주석처리 유지
