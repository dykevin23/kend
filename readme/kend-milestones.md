...
>
> **2026-09-17 갱신** (공유문서 sync 반영): kend-seller Phase 3 진척 대량 반영 — **B-1 상품수정·B-2 재고관리(Stocks Keeping)·B-3 Seller 대시보드+디자인시스템 리뉴얼·B-7 리뷰관리 전부 완료·실사용 테스트 통과** 확인(트래커가 그동안 stale했던 것 정정). 판매자 self-service 정산조회 화면은 구현됨(테스트 대기)으로 신규 추가. **B-5 공지사항도 완료** — kend-seller 스키마+admin CRUD(+target 대상구분) 완료, kend 조회화면(`/myPage/notices`) 연결까지 마무리. 남은 kend-seller 항목은 B-4(CS관리 잔여)·B-6(교환, 정책대기) 뿐
>
...
- **CS관리 운영기능 잔여** (B-4) — 카테고리·상태 필터, 미답변 표시(대시보드 KPI+사이드바 배지)는 **이미 구현돼 있었음**(P2.5-4 2026-08-25, 09-17 대시보드) 확인, 실제 남은 건 문의 목록 **기간(날짜) 필터·정렬**뿐. 담당자배정/응답률통계는 저우선 선택사항
...
- ✅ **공지사항 화면 (B-5)** — kend-seller가 `notices` 스키마+admin CRUD(등록/노출토글/삭제) 완료, `target`(ALL/SELLER/BUYER) 대상 구분 컬럼 추가. kend은 `/myPage/notices`에 로더 연결해 노출중+BUYER·ALL 대상만 아코디언으로 표시(제목 "공지사항 및 FAQ"→"공지사항"으로 정정) — kend-seller 작업 전까지 막혀있던 항목 해소 (2026-09-17)
