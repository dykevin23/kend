...
>
> **2026-09-17 갱신** (공유문서 sync 반영, 3차): kend-seller Phase 3 진척 대량 반영 — **B-1 상품수정·B-2 재고관리(Stocks Keeping)·B-3 Seller 대시보드+디자인시스템 리뉴얼·B-5 공지사항·B-7 리뷰관리 전부 완료·실사용 테스트 통과** 확인(트래커가 그동안 stale했던 것 정정, B-5는 테스트대기→완료로 두 번 정정 끝에 확정). **B-4(CS관리)는 구현 완료, 실사용 클릭 테스트만 대기**. 판매자 self-service 정산조회 화면도 구현됨(테스트 대기). 남은 kend-seller 항목은 B-6(교환, 정책대기) 뿐
>
> **2026-09-21 갱신**: ✅ Phase 3 **"최근 본 상품"(kend)** 구현·실사용 테스트 통과로 완료 — 조회 기록을 상품상세 loader에 뒀다가 `ProductCard`의 `prefetch="intent"`로 오탐지되는 버그를 발견해 컴포넌트 마운트 후 액션으로 분리(향후 "조회수/방문 기록" 류 기능은 loader가 아니라 이 패턴을 따를 것). **팔로워 수** 항목은 드롭 결정 + `follows` 테이블/`profiles.stats` 실제 DB drop 완료. 상품상세 타이틀 옆 별점/리뷰수 하드코딩(09-07에 놓쳤던 블록) 추가 발견·수정. 남은 Phase 3: CS관리(B-4)·정산조회 화면 테스트 대기, 상품 카테고리/태그필터·사이즈표 미착수, 교환(B-6) 정책 대기
...
- ✅ **최근 본 상품 (kend)** — `product_views` 테이블(찜 테이블과 동일한 user_id+product_id 복합키 패턴, 조회 로그 아님) 신설, `recent-products-page.tsx` 실제 목록으로 구현. 조회 기록을 상품상세 loader가 아니라 컴포넌트 마운트 후 별도 액션으로 분리 — `ProductCard`의 `prefetch="intent"`만으로 기록되던 버그를 실사용 테스트로 발견·수정. 실사용 재검증 통과 (2026-09-21)
- ❌ **팔로워 수 — 드롭 + DB 정리** — `follows` 테이블(예전 사람-to-사람 팔로우 기능 잔재, 실사용 코드 0건)·`profiles.stats` 컬럼 DB drop 완료. 스토어 카드의 하트+숫자 UI는 원래 팔로우가 아니라 찜하기/조회수 의도였던 것으로 확인돼 UI는 유지, `likeCount`로 명명만 정리(집계 구현은 미착수, `store_likes` 기반으로 추후) (2026-09-21)
- ✅ **공지사항 화면 (B-5)** — kend-seller `notices` 스키마+admin CRUD(등록/노출토글/삭제) + `target`(ALL/SELLER/BUYER) 대상구분 + 스펙에 없던 판매자 본인용 조회 화면(`/seller/notices`)까지 추가 구현. kend `/myPage/notices`는 target 필터 연결(SELLER 전용 제외), 제목 "공지사항"으로 정정. **kend-seller 실사용 테스트 완료(사용자 확인)** — 09-17 한 차례 테스트대기로 정정했다가 seller 쪽 changelog 갱신으로 재확인 (2026-09-17)
...
- ✅ 상품 평점/만족도 가짜 데이터 — 평점·리뷰수 실데이터로 교체, 색감/사이즈/두께감 만족도는 데이터 없어 제거(백로그로 별도 관리 안 함, 필요 시 재검토) (2026-09-02). **추가 발견분**: 상품상세 타이틀 옆에 별도로 하드코딩돼 있던 "4.6·리뷰(4,321)" 블록을 이때 놓쳤던 것 확인, 실데이터로 교체 + 타이틀과 한 줄 우측정렬로 레이아웃 조정 — 구현됨, 실사용 확인 대기 (2026-09-21)
...
*최종 갱신: 2026-09-21 (최근 본 상품 기능 구현 완료, 팔로워 수 드롭+DB 정리 완료, 상품상세 별점/리뷰수 하드코딩 추가 발견분 수정)*
