...
- **리뷰 관리** (구 P2-10 잔여)
- ⚠️ **교환(exchange, 구 P2.5-3 하위 항목)** — 2026-08-25, Phase 2.5 종료 처리하며 이관. **이 목록의 다른 항목과 달리 드롭 가능한 편의기능이 아니라 구매자 대상 핵심 거래 기능**(반품과 한 가족) — 개발이 밀린 게 아니라 정책(옵션재고 없을때 처리, 배송비 부담주체, 횟수제한 등)이 안 정해져서 착수를 못 하는 상태. 정책 결정되는 대로 우선 착수 필요, Phase 3의 다른 항목과 우선순위를 동일시하면 안 됨
...
- **선행**: EXT-1 (법인 완료) ✅, EXT-7(Toss 지급대행 신청/승인)
- **범위 변경(2026-08-31)**: 장기적으로 Toss 지급대행 API 이용 예정이나, 결제 실키 승인(EXT-5) 후에야 지급대행 신청이 가능 → **지급대행 연동 전 단계까지만** 이번 라운드에서 진행하고, 실제 Toss 셀러 등록/KYC API 연동은 EXT-7 승인 이후로 분리. 그 전까지는 수동 송금으로 운영(P3.5-2가 계산만 하고 송금은 안 함)

#### 🟡 P3.5-2. 정산 계산 배치 (구 P3-3) — **kend-seller 작업**
- **선행**: P2.5-2(구매확정), P2.5-3(반품/환불 반영), P2.5-5(`shipping_fee_bearer`) — 이 셋의 데이터가 정산 계산의 입력값(전부 kend 완료)
- **포함**: settlement_items 테이블(kend-seller 소유), 정산 항목 생성 cron, 수수료 차감(`platform_settings.commission_rate` 자체 스키마이므로 크로스 스키마 read 불필요), 주기 설정(월 1회)
...
#### 🟡 P3.5-3. 정산 내역 조회 (구 P3-4) — **kend-seller 작업**
- **포함**: 내역 목록(기간 필터), 주문별 명세(별도 테이블 없이 delivery_items 재조회로 구성), 상태를 pending→paid로 표시하는 액션, **화면은 admin 전용**(2026-08-31 확정 — pending→paid 액션이 admin 전용이라 조회도 같은 사람. 판매자 셀프조회는 별도 후속 작업)
- **백로그로 이관(2026-08-31)**: 엑셀 다운로드 — 구 스펙에 있던 항목을 검증 없이 옮겨온 것, 구체적 필요(세무처리 등) 확인 전까지 이번 라운드 스코프 아님
