...
> **2026-05-07 메모 → 2026-09-09 갱신**: swipe back UX용으로 `makeCachedClientLoader`(URL 단위 clientLoader 캐시)를 `/stores`·`/stores/:storeId`에 임시 도입했으나, **영구 캐시가 "스토어 목록 stale" 버그를 유발**해 방향 전환. 헬퍼 삭제 + 전 라우트 항상 신선 + 뒤로가기 잔상은 렌더링 레이어에서 해결로 재확정 → [data-freshness-caching-policy.md](./data-freshness-caching-policy.md). 본 문서의 "clientLoader 캐싱은 React Query 재발명" 입장은 여전히 유효, 그 5-Phase 로드맵의 Phase 6이 곧 이 문서(React Query 전환)다.
