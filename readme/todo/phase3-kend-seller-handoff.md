...
> **2026-09-17 갱신 (3차)**: B-1/B-2/B-3/B-5/B-7 전부 실사용 테스트까지 완료 확인(B-5는 테스트대기→완료로 seller changelog 갱신 후 재확인). **B-4만 구현됨/테스트 대기.** 남은 건 B-6(정책대기)뿐.

| 항목 | 소유 | 상태 | 비고 |
|---|---|---|---|
| ~~상품 수정 기능~~ | kend-seller | ✅ 완료(09-14) | → §B-1 |
| ~~공지사항 스키마 + admin CRUD~~ | kend-seller → kend | ✅ 완료(09-17) | kend 조회화면 연결까지 실사용 테스트 통과 → §B-5 |
| **교환(exchange)** | kend + kend-seller | 정책 대기 ⚠️ | 드롭 불가 핵심 거래기능. 정책 미정으로 착수 불가 → §B-6 |
| ~~판매자 수동 재고 조정 ("Stocks Keeping")~~ | kend-seller | ✅ 완료(09-16) | → §B-2 |
| ~~Seller 대시보드 (판매 통계)~~ | kend-seller | ✅ 완료(09-17, 디자인시스템 리뉴얼 포함) | → §B-3. 신규 범위: 판매자 self-service 정산조회 화면은 구현됨/테스트 대기 |
| CS관리 운영기능 (필터/담당자/통계) | kend-seller | 🔄 구현됨, 테스트 대기 | 기간필터+정렬+통계카드 3개로 잔여범위 구현 완료, 클릭 테스트 전 → §B-4 |
| ~~리뷰 관리 (답변/통계/필터)~~ | kend-seller | ✅ 완료(09-07) | kend 쪽도 완료. → §B-7 |
...
**요약**: kend-seller 소유 6개 중 5개(상품수정·재고조정·대시보드·공지스키마·리뷰관리) 실사용 테스트까지 완료, CS관리(B-4)는 구현 완료·테스트 대기. 남은 건 교환절반(B-6, 정책대기)뿐. 나머지는 kend / kend-native.
...
### B-4. CS관리 운영기능 — 구현됨, 테스트 대기 (2026-09-17)

**구현 내용**: `/orders/inquiries`에 접수일 기간 필터(리뷰관리 화면의 date input 패턴 재사용), 정렬(최신순/답변대기 우선), 전체·미답변·응답률 통계 카드 3개 추가. 담당자 배정은 단일 판매자 계정 구조상 실효성 낮다고 판단해 스코프 제외(kend-seller 자체 판단).

**남은 것**: kend-seller 자체 changelog에 "아직 실사용 화면 클릭 테스트 전"이라고 명시 — 클릭 테스트 확인되면 완료.

**kend 영향**: 없음. kend는 단일 질문+단일 답변 조회만.

---

### B-5. 공지사항 스키마 + admin CRUD ✅ 완료 (2026-09-17)

**완료 내용**
- kend-seller: `notices` 테이블(`title`/`content`/`is_visible`/`target`/`created_at`/`updated_at`) + `/system/notices` admin CRUD(등록/노출토글/삭제, `admin-layout` 재사용). 당초 요청 스펙에 없던 **`target` enum(`ALL`/`SELLER`/`BUYER`) 컬럼**과, 역시 스펙 밖이던 **판매자 본인용 조회 화면(`/seller/notices`, SELLER/ALL 대상만)까지 kend-seller가 자체적으로 추가**
- kend: `/myPage/notices` 로더 연결, `is_visible=true` + `target IN (ALL, BUYER)`인 공지만 최신순 아코디언으로 노출(SELLER 전용 공지는 kend에서 제외). 페이지 제목 "공지사항 및 FAQ" → "공지사항"으로 정정(FAQ 기능 없어서)
- kend-seller 실사용 테스트 완료(사용자 확인) — 관리자 등록/노출토글/삭제, 판매자 조회화면·대시보드 카드까지 전부 확인

**kend 영향**: 없음(완료). 문의하기(P2.5-4)와 동일한 소유권 구조 그대로 유지 — kend-seller가 스키마+CRUD, kend는 조회만.
