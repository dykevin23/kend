...
> **2026-09-17 갱신**: B-1/B-2/B-3/B-5/B-7 전부 완료 확인(공유문서 sync로 kend-seller changelog 반영). 아래 표는 최초 인계 시점(09-10) 기준 원본이라 완료분은 취소선 처리, 남은 건 B-4/B-6뿐.

| 항목 | 소유 | 상태 | 비고 |
|---|---|---|---|
| ~~상품 수정 기능~~ | kend-seller | ✅ 완료(09-14) | → §B-1 |
| ~~공지사항 스키마 + admin CRUD~~ | kend-seller → kend | ✅ 완료(09-17) | kend 조회화면 연결까지 완료 → §B-5 |
...
**요약**: kend-seller 소유 6개 중 4개(상품수정·재고조정·대시보드·리뷰관리) + 공지스키마(B-5) 완료. 남은 건 CS관리(B-4)·교환절반(B-6)뿐. 나머지는 kend / kend-native.
...
### B-4. CS관리 운영기능 — 잔여 범위 축소 (2026-09-17 정정)

**현상**: 문의하기 코어(P2.5-4, 2026-08-25)에 이미 **카테고리·상태(답변대기/답변완료) 필터**가 구현돼 있었음 — 이 문서 작성 시점(09-10)에 이를 놓치고 "운영 레이어 미구현"으로 잘못 기재. 09-17 Seller 대시보드 작업으로 "미답변 일반문의" KPI(관리자)·사이드바 미답변 배지도 추가돼 미답변 하이라이트도 사실상 충족.

**요청 작업 (실제 남은 것만)**
- 판매자 문의 목록(`/orders/inquiries`)에 **기간(날짜) 필터** 추가 (카테고리·상태 필터는 이미 있음)
- 정렬 옵션
- (선택, 저우선) 담당자 배정, 응답률 통계
...
**완료 내용**
- kend-seller: `notices` 테이블(`title`/`content`/`is_visible`/`target`/`created_at`/`updated_at`) + `/system/notices` admin CRUD(등록/노출토글/삭제, `admin-layout` 재사용). 당초 요청 스펙에 없던 **`target` enum(`ALL`/`SELLER`/`BUYER`) 컬럼을 kend-seller가 추가**해 공지 대상(전체/판매자전용/구매자전용)을 구분하도록 확장
